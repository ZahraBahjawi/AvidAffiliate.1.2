    ...
    <section className="py-10">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-8">
          <p className="text-lg text-gray-300 font-light">On average, our full audits find:</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          <div className="bg-slate-800/70 border border-slate-700 rounded-xl p-6 text-center">
            ...
            <p className="text-gray-300 text-sm">unmonetized mentions across ~50 different brands.</p>
          </div>
          <div className="bg-slate-800/70 border border-slate-700 rounded-xl p-6 text-center">
            <p className="text-3xl font-bold text-orange-400 mb-1">~250</p>
            <p className="text-gray-300 text-sm">broken links.</p>
          </div>
          <div className="bg-slate-800/70 border border-slate-700 rounded-xl p-6 text-center">
            <p className="text-3xl font-bold text-amber-400 mb-1">Est. 20% uplift.</p>
            <p className="text-gray-300 text-sm">in affiliate revenue*</p>
          </div>
        </div>
      </div>
...
const ServicesFlow: React.FC = () => {
  return (
    <div className="mt-12">
      {/* Services flow removed */}
    </div>
  );
};
  ...
  return (
    <div className="space-y-3">
      {faqItems.map((item, index) => (
        <div key={index} className="bg-gradient-to-r from-orange-900/30 to-red-900/30 rounded-xl border border-orange-700/50 shadow-lg">
          <button
            onClick={() => handleToggle(index)}
            className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-orange-900/20 transition-all duration-200 rounded-xl group focus:outline-none focus:ring-2 focus:ring-orange-400 focus:ring-offset-2 focus:ring-offset-slate-700"
            aria-expanded={openItem === index}
            aria-controls={`faq-answer-${index}`}
            id={`faq-question-${index}`}
          >
            <span className="text-lg font-medium text-white group-hover:text-orange-300 transition-colors">{item.q}</span>
            <div className={`transform transition-all duration-200 ${openItem === index ? 'rotate-180 text-orange-400' : 'text-gray-400 group-hover:text-orange-300'}`}>
          ...
          {openItem === index && (
            <div 
              className="px-6 pb-4 border-t border-orange-700/30"
              id={`faq-answer-${index}`}
              role="region"
...
const JSONLDSnippets: React.FC = () => {
  const currentDomain = typeof window !== 'undefined' ? window.location.origin : 'https://avidaffiliate.com';
  
  const org = {
    '@context': 'https://schema.org',
    '@type': 'Organization',
    name: 'AvidAffiliate',
    description: 'Affiliate revenue optimization platform helping content creators unlock hidden monetization opportunities',
    logo: `${currentDomain}/LOGO.png`,
    url: currentDomain,
    sameAs: ['https://www.linkedin.com/company/avidaffiliate'],
  };
  ...
  const website = {
    ...
    name: 'AvidAffiliate',
    url: currentDomain,
    description: 'Free affiliate link audit and revenue optimization platform for content creators',
  };
  ...
  const faqItems = [
    ...
    {
      q: 'What\'s included in the report card?',
      a: 'A prioritized list of unmonetized mentions, broken/redirecting links, and recommended higher‑paying programs—plus estimated impact and next steps.',
    },
    {
      q: 'What access do you need?',
      a: 'Only your public website URL. Our crawler is read‑only and does not require credentials or code access.',
    ...
    {
      q: 'What happens after I get the report card?',
      a: 'Once we share your report card, we will find time with you to discuss your goals, share more details and develop an action plan.',
    },
    {
      q: 'What happens after the report card?',
      a: 'Choose your path: we can implement fixes (Implement), recommend higher‑paying programs (Discover), or build a 90‑day monetization plan (Strategize).',
    },
    {
      q: 'Will this affect my site performance?',
      a: 'No. Our crawler is read-only and designed to have zero impact on your site\'s performance or user experience.',
    ...
    {
      q: 'Do you guarantee results?',
      a: 'While we can\'t guarantee specific revenue increases, our report cards consistently identify actionable opportunities. Most clients see measurable improvements within 30 days of implementation.',
    },
  ];
...
const ReportcardPreview: React.FC = () => {
  ...
  return (
    <div className="mt-8 max-w-2xl mx-auto">
      <p className="text-center text-gray-300 text-sm mb-4">Preview a sample report card</p>
      
      <div className="bg-slate-800 rounded-lg border border-slate-600 p-6 relative">
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-colors ${
                index === currentSlide ? 'bg-orange-400' : 'bg-slate-600'
              }`}
...
const AboutUsPage: React.FC<AboutUsPageProps> = ({ onNext = () => {}, onNavigate = () => {}, onBack = () => {} }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-900" style={{ fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      {/* Hero Section */}
      <section id="main-content" className="pt-16 pb-10 bg-slate-900">
        <div className="max-w-7xl mx-auto px-6">
